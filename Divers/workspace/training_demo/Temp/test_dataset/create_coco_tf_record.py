...
def load_coco_dection_dataset(imgs_dir, annotations_filepath, shuffle_img = True ):
    ...
    for index, img_id in enumerate(img_ids):
        ...
        for ann in anns:
            ...
            bboxes_data = [int(bboxes_data[0])/float(pic_width), int(bboxes_data[1])/float(pic_height),\
                                  int(bboxes_data[2])/float(pic_width), int(bboxes_data[3])/float(pic_height)]
...
def main(_):
    if FLAGS.set == "train":
        ...
        annotations_filepath = os.path.join(FLAGS.data_dir,'annotations','train.json')
        print("Convert coco train file to tf record")
    elif FLAGS.set == "val":
        imgs_dir = os.path.join(FLAGS.data_dir, 'val')
        annotations_filepath = os.path.join(FLAGS.data_dir,'annotations','val.json')
    ...
    with tf.python_io.TFRecordWriter('output/'+FLAGS.set+'.tfrecord') as tfrecord_writer:
